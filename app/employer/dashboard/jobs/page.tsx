...
export default function JobsClient() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        setIsLoading(true);
        // 🚀 Fetch from HR route (which has application_count) and bypass pagination!
  ...
  const jobs = useMemo(() => {
    return rawJobs.map((j) => {
      ...
      return {
        ...
        title: titleMatch ? titleMatch[1].trim() : formatTitleCase(j.job_rank),
        rank: formatTitleCase(j.job_rank),
        ...
        // 🚀 Now accurately pulls the count from your HR backend response
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${STATUS_BADGE[job.status] || STATUS_BADGE["active"]}`}
                    >
                ...
                <div className="flex shrink-0 items-center gap-5">
                  <div className="text-center">
                    <p className={`text-lg font-extrabold ${job.status === 'closed' ? 'text-slate-400' : 'text-[#0F1E35]'}`}>{job.applicationCount}</p>
                  ...
                  {job.newCount > 0 && (
                    <div className="text-center">
                      <p className={`text-lg font-extrabold text-[#F5B61A]`}>{job.newCount}</p>
                      <p className="text-[10px] font-medium uppercase tracking-wide text-slate-400">
                        New
                      </p>
                    </div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-[#E7EAF1] pt-4">
                ...
                {job.status === "active" ? (
                  <button 
                    ...
                    className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-xs font-bold text-red-600 transition-colors hover:bg-red-100 hover:text-red-700 shadow-sm"
                  >
                    Close Job
                  </button>
                ) : (
                  <button 
                    ...
                    className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-bold text-emerald-700 transition-colors hover:bg-emerald-100 hover:text-emerald-800 shadow-sm"
                  >
                    Reopen Job
                  </button>
                )}
