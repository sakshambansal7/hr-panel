...
import { Mail, Phone, Briefcase, CheckCircle2, XCircle } from "lucide-react";
...
export default function InterviewsPage() {
  ...
  // 🚀 HARD-FILTERED FOR 'INTERVIEWED' ONLY
    ...
    <DashboardShell pageTitle="Interview Pipeline">
      <div className="mb-8">
        ...
        <p className="text-sm font-medium text-slate-500">
          Candidates actively participating in the interview process.
        </p>
      </div>
      ...
      <div className="rounded-[20px] border border-[#E7EAF1] bg-white shadow-sm overflow-hidden">
        <div className="p-5 border-b border-[#E7EAF1] bg-slate-50/50 flex flex-wrap items-center justify-between gap-4">
           <h2 className="text-sm font-bold text-[#0F1E35]">In Progress ({interviewCandidates.length})</h2>
           <div className="flex items-center gap-2">
             <Briefcase className="h-4 w-4 text-slate-400" />
             <select 
               value={selectedJobId}
               onChange={(e) => setSelectedJobId(e.target.value)}
               className="text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-purple-500 max-w-[200px] truncate"
             >
                ...
                {interviewCandidates.map((app) => (
                  <tr key={app.application_id || app.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button 
                        onClick={() => updateStatus(app.application_id || app.id, 'selected')}
                        className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 hover:bg-emerald-100 transition-colors"
                      >
                        <CheckCircle2 className="h-3.5 w-3.5" /> Mark Selected
                      </button>
                      <button 
