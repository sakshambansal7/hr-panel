...
function SearchableSelect({
  ...
  const filtered = useMemo(() => {
    ...
    // 🚀 FIXED: Strip everything except letters and numbers for a bulletproof search
    const cleanQuery = query.toLowerCase().replace(/[^a-z0-9]/g, ''); 
    
    console.log(`[Search Debug] Typing: "${query}" -> Cleaned: "${cleanQuery}"`);
    ...
    console.log(`[Search Debug] Found ${results.length} matches for "${query}"`);
    return results;
...
export default function SignupForm() {
  ...
  useEffect(() => {
    // 🚀 Ab backend limit=all ko samjhega aur saari companies ek baar me bhej dega!
  ...
  async function handleSendOtp() {
    ...
    try {
      ...
      const payload = {
        ...
        // 🚀 FIXED: Added the required country code (+91)
        phone_number: `+91${mobileNumber.trim()}`, 
        
        password: password,
        
        // 🚀 FIXED: Added confirmPassword which your backend validation schema requires
  ...
  async function handleResendOtp() {
    ...
    try {
      setIsLoading(true);
      // 🚀 Using your exact backend route for resending registration OTP
